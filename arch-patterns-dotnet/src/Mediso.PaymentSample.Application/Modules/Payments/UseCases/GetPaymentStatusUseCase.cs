...
{
    public static async Task<PaymentStatusResponse?> Handle(GetPaymentStatusQuery query, IQuerySession session,
    [FromServices]
    ILogger<GetPaymentStatusUseCase> logger)
    {
        ...
        try
        {
            if (string.IsNullOrWhiteSpace(query.CorrelationId))
            {
                ...
                throw new NullReferenceException("CorrelationId is null");
            }
            ...
            if (saga is null)
            {
                ...
                if (payments.Count > 0)
                {
                    ...
                    return new PaymentCompletedStatusResponse
                    {
